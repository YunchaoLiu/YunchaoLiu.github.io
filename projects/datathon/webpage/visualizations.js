...
function plot_it() {
    ...
    let scaling_factor = 190000 // The actual number divide by this scaling factor and then shows on the plot


    // aggregate the data
    let aggregatedByState = d3.nest().key(function (d) {
        return d.statename;
    }).entries(treatment_data);

    console.log("ArrayData :", aggregatedByState);
    console.log("ArrayData[0] :", aggregatedByState[0]);
    ...
    // function convert_state_key(state_key) {
    //       case 1:
    //         return 1
    // }
    ...
    let states = zoomZone.selectAll(".state")
        ...
        .style("fill", function (d) {
                ...
                return d3.interpolate(lightYellowish, darkReddish)(coef / scaling_factor);
            }
        );
    ...
    createButton("Show Bases", 2, function () {
        if (!view_air_bases) {
            view_air_bases = true;
            plotAirBases();
        } else {
    ...
    createButton("Re-center map", 3, recenter, orangish);
    ...
    // let textDataLegend = [0, maxCount / 10000, maxCount / 5000, maxCount / 500, maxCount / 50, maxCount];
    let textDataLegend = [0, 30000, 60000, 120000, 250000, 500000];
    let colorDataLegend = textDataLegend.map(each => {
        return d3.interpolate(lightYellowish, darkReddish)(each / scaling_factor);
    });
